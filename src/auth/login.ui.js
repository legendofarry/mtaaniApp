...
export const renderLogin = () => {
      ...
      <div class="auth-card">
        ...
        <form id="login-form">
          ...
          <div style="justify-content:center;display:flex;margin-bottom:20px">
              <button id="biometric-btn" title="Use biometrics" class="text-gray-500" style="background:transparent;border:none;cursor:pointer">
                <svg xmlns="http://www.w3.org/2000/svg" id="Layer_1" data-name="Layer 1" viewBox="0 0 24 24" class="w-12 h-12">
  ...
  biometricBtn.addEventListener("click", async (e) => {
    ...
    try {
      ...
      if (!biometricService || !biometricService.authenticateWithPasskey) {
        showToast("Biometrics not available in this browser", "warning");
        return;
      }

      const r = await biometricService.authenticateWithPasskey();
      if (!r.success) {
        // If no saved creds, let user know they should enable after login
        showToast(
          "No saved biometrics found on this device. Sign in and enable biometrics from your profile.",
          "info"
        );
        return;
      }
