...
export const confirm = (message, title = "Confirm") => {
  return new Promise((resolve) => {
    ...
    box.innerHTML = `
      ...
      <div style="color:#374151;margin-bottom:16px">${message}</div>
      <div style="display:flex;justify-content:flex-end;gap:8px">
        <button id="modal-no" style="padding:8px 12px;border-radius:8px;border:1px solid #e5e7eb;background:#fff">No</button>
        <button id="modal-yes" style="padding:8px 12px;border-radius:8px;background:var(--primary-color);color:#fff">Yes</button>
      </div>
    `;
